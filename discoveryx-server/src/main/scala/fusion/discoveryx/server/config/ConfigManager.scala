...
import fusion.discoveryx.server.config.internal.ConfigManagerBehavior
...
object ConfigManager {
  ...
  private def apply(entityId: String): Behavior[Command] =
    Behaviors.setup(context => new ConfigManagerBehavior(entityId, context).eventSourcedBehavior())
}
